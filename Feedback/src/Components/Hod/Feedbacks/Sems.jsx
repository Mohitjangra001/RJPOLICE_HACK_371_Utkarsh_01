...
const Sems = () => {
    ...
    const addsem = async (e) => {
        ...
        if (data.success) {
            toast.success("District added succesfully")
            getsems()
        }
    }
    ...
    const delsem = async (id) => {
        ...
        if (confirmed === "del") {
            ...
            if (data.success) {
                toast.success("District deleted succesfully")
                getsems()
            }
        }
        else {
        ...
        <div className={`${theme == "light" ? " bg-white text-black" : "bg-[#1d232a]"} h-[91vh] overflow-y-auto p-5 w-full`}>
            <section className='flex justify-between items-center'>
                ...
                <section className=' w-[20%]'>
                    <button onClick={() => window.my_modal_1.showModal()} className='px-7 py-1 font-bold  shadow-md hidden sm:block text-white bg-blue-700 rounded-full'> New Station</button>
                </section>
            </section>
            <div className='mt-5 '>
            ...
            <dialog id="my_modal_1" className="modal">
                <form method='dialog' className={`modal-box ${theme == 'dark' ? " text-white bg-[#1d232a]" : "text-blsck bg-white"}`}>
                    ...
                    <h1 className='text-blue-700 font-bold'>Add Station No.</h1>
