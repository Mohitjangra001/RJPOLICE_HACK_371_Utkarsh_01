...
const teacher = () => {
    ...
    const getst = async (e) => {
        ...
        setploader(true)
        try {
            const { data } = await axios.post("http://localhost:3000/api/v2/searchfac", {
                search: search
            });
            if (data.success) {
                console.log(data)
    ...
    const addfac = async (e) => {
        e.preventDefault();
        try {
            const { data } = await axios.post("http://localhost:3000/api/v2/addfac", {
                name: name,
                email: email,
                phone: phone,
                education: education,
            ...
            if (data.success) {
                toast.success(" Senior Offices Added Succesfully  ", {
                    autoClose: 2000,
                });
                getteachers()
            }
    ...
    const upfac = async (e) => {
        e.preventDefault();
        try {
            const { data } = await axios.put("http://localhost:3000/api/v2/updateteacher", {
                name: name,
                email: email,
                phone: phone,
                education: education,
            ...
            console.log(data)
            if (data.success) {
                toast.success(" Senior Officers updated Succesfully  ", {
                    autoClose: 2000,
                });
                getteachers()
            }
    ...
    const delfac = async (id) => {
        ...
        if (confirmed) {
            console.log(id);
            const { data } = await axios.delete(`http://localhost:3000/api/v2/delfac/${id}`);
            if (data.success) {
                toast.success("Policer deleted succesfully")
                getteachers()
            }
        }
    ...
    const getbyid = async (id) => {
        setploader(true)
        console.log(id)
        const { data } = await axios.get(`http://localhost:3000/api/v2/fac/${id}`)
    ...
    return (
        <div className={`p-2 h-[91vh] overflow-y-auto sm:p-5 ${theme == "light" ? "bg-white " : "bg-[#1d232a]"}`}>
            <div className='flex items-center justify-between flex-col sm:flex-row w-full'>
                <section>
                    <h1 onClick={() => window.location.reload()} className={`text-2xl cursor-pointer font-bold ${theme == "light" ? "text-black " : "text-white"}`}>Officer </h1>
                </section>
                <section className='flex w-[90%] p-2 sm:[50%] justify-center items-center'>
                    <form className='w-[80%] ' action="" onSubmit={getst}>
                        <input type='text' placeholder='Search For Officer' className={`rounded-full px-5 p-1 w-[100%] ${theme == "light" ? "bg-white border-b-2  text-black" : "bg-[#0c131d]"}`}
                            value={search} onChange={handlesearchchange}
                        />
                    </form>
                </section>
                <section className=' w-[20%]'>
                    <button onClick={() => window.my_modal_1.showModal()} className='px-7 py-1 font-bold hidden sm:block text-white bg-blue-700 rounded-full'> New Officer</button>
                </section>

            </div>

            <button onClick={() => window.my_modal_1.showModal()} className='px-3 mt-5 py-1  shadow-lg sm:hidden absolute bottom-1 right-2  text-white font-semibold bg-blue-700 rounded-full'> New Officer +</button>
            <div className='flex justify-center'>
                {ploader ? (
                    ...
                    <section className='flex justify-center items-center h-[70vh]'>
                        <h1 className=''>No  officer found</h1>
                    </section>
            ...
            <dialog id="my_modal_4" className="modal">
                <form method='dialog' className={`modal-box ${theme == 'dark' ? " text-white bg-[#1d232a]" : "text-blsck bg-white"}`}>
                    ...
                    <h1 className='text-blue-700 font-bold text-xl'>Update Officer</h1>
            ...
            <dialog id="my_modal_1" className="modal">
                <form method='dialog' className={`modal-box ${theme == 'dark' ? " text-white bg-[#1d232a]" : "text-blsck bg-white"}`}>
                    <button className={`btn btn-sm btn-circle btn-ghost absolute right-2 top-2 ${theme == 'dark' ? " text-white bg-black" : ""}`}>✕</button>
                    <h1 className='text-blue-700 font-bold text-xl'>Add Officer</h1>
                    <div className='w-full mt-2'>
                    ...
                    {ploader ?
                        ...
                        <>
                            ...
                            <div className={`${theme == 'dark' ? " focus:outline-none border-none text-white " : "text-black bg-white"}`}>
                                ...
                                <h1 className='text-center'>Seniour Officer Profile</h1>
                                <hr></hr>
