...
const Subjects = () => {
    ...
    const getTeachers = async () => {
        try {
            getUserData()
            setLoading(true)
            const { data } = await axios.post("http://localhost:3000/api/v2/fac", {
                dep: dep
            })
    ...
    const getSubjects = async () => {
        try {
            setLoading(true)
            const { data } = await axios.post("http://localhost:3000/api/v2/subjects", {
                dep: dep
            })
            console.log(data)
    ...
    const upteacher = async (id, value) => {
        ...
        const { data } = await axios.put("http://localhost:3000/api/v2/uteacher", {
            id: id, value: value
        });
        if (data?.success) {
            getSubjects();
            toast.success('Officer updated succesfully.', {
                position: "top-center"
            })
        }
        else {
    ...
    const getbyfilter = async (sem) => {
        setLoading(true)
        sets(sem)
        const { data } = await axios.post("http://localhost:3000/api/v2/getsubbysem", {
            sem: sem
        });
    ...
    const addsub = async (e) => {
        ...
        const { data } = await axios.post("http://localhost:3000/api/v2/course", {
            name: name,
            department: dep,
            sem: sem,
            teacher: teacher
    ...
    const searchsub = async (e) => {
        e.preventDefault();
        setLoading(true)
        const { data } = await axios.post("http://localhost:3000/api/v2/searchsub", {
            search: search
        });
    ...
    return (
        <div className={`p-2 h-[91vh] overflow-y-auto sm:p-5 ${theme == "light" ? "bg-white " : "bg-[#1d232a]"}`}>
            ...
            <div className='flex items-center justify-between'>
                <section>
                    <h1 onClick={() => window.location.reload()} className={`text-2xl cursor-pointer font-bold ${theme == "light" ? "text-black " : "text-white"}`}>Services </h1>
                </section>
                <section className='flex w-[100%] sm:w-[60%] p-2 sm:[50%] justify-center items-center'>
                    <form className='w-[100%] ' action="" onSubmit={searchsub} >
                        <input type='text' placeholder='Search For Station Officer' value={search} onChange={handlesearchchange} className={`rounded-full px-5 p-1 w-[100%] ${theme == "light" ? "bg-white border-b-2  text-black" : "bg-[#0c131d]"}`}

                        />
                    </form>
                </section>
                <section className='w-[20%] '>
                    <button className='px-7 py-1 hidden sm:block text-white  bg-blue-700 rounded-full' onClick={() => window.my_modal_1.showModal()} >New Services</button>
                </section>
                <button onClick={() => window.my_modal_1.showModal()} className='px-7 py-1 sm:hidden absolute bottom-1 right-2  text-white font-semibold bg-blue-700 rounded-full'> New Services +</button>
                ...
                <div className={`py-3 sm:p-6 flex justify-center overflow-x-auto mt-5 ${theme == "light" ? "text-black" : "text-white"}`}>
                    <table className='border-collapse select-none w-[90%] '>
                        <thead>
                            <tr className=' bg-blue-700 rounded-md'>
                                ...
                                <th className='p-2 text-left py-2 text-white text-lg'>Station</th>
                                <th className='p-2 text-left py-2 text-white text-lg'>Officer</th>
                                <th className='p-2 text-left py-2 text-white text-lg'> Update officer</th>
                        ...
                        {subjects.map((item, index) => (
                            <>
                                <tr className={`${theme == "light" ? "hover:bg-gray-300 " : "hover:bg-slate-950"}`} key={index}>
                                    ...
                                    <Select className='w-full ant-input text-xl mb-2 mt-2 rounded-2xl ' placeholder='Select Officer' onChange={(value) => upteacher(item._id, value)} >
                                        {teachers.map((s) => (
                    ...
                    <dialog id="my_modal_1" className="modal">
                        <form method='dialog' className={`modal-box ${theme == 'dark' ? " text-white bg-[#1d232a]" : "text-black bg-white"}`}>
                            ...
                            <h1 className='text-blue-700 font-bold'>Add Service</h1>
                            <div className='w-full mt-2'>
                                <form className='w-[1005]'>
                                    ...
                                    {<select className={`p-2 my-2 w-full rounded-full ${theme == "light" ? "bg-white text-black   " : " bg-[#0c131d] focus:outline-none border-none"}' `} placeholder='select a Officer ' onChange={(e) => handlet(e.target.value)}>
                                        <option>select a Officer</option>
                                    ...
                                    {<select className={`p-2 my-2 w-full rounded-full ${theme == "light" ? "bg-white   " : " bg-[#0c131d] focus:outline-none border-none"}' `} placeholder='select a District ' onChange={(e) => handles(e.target.value)}>
