...
const Manage = () => {
    ...
    const delhod = async (id) => {
        ...
        if (pass === "del") {
            ...
            toast.success("Manager Deleted Succesfully")
            alldepartments()
            gethods()
        }
        else {
    ...
    return (
        <div className={`p-2 h-[91vh] overflow-y-auto  sm:p-5 ${theme == "light" ? "bg-white " : "bg-[#1d232a]"}`}>
                        ...
                        <table className='border-collapse select-none w-[90%] overflow-x-auto '>
                            <thead>
                                <tr className=' bg-blue-700 rounded-md'>
                                    ...
                                    <th className='p-2 text-left py-2 text-white text-lg'>Manager</th>
                                    <th className='p-2 text-left py-2 text-white text-lg'> Update Manager</th>
            ...
            <button onClick={() => window.my_modal_2.showModal()} className='px-7 py-1 font-bold   absolute bottom-5 right-10 text-sm  text-white bg-blue-700 rounded-full'> + Add Manager </button>
            ...
            <dialog id="my_modal_2" className="modal">
                <form method='dialog' className={`modal-box ${theme == 'dark' ? " text-white bg-[#1d232a]" : "text-blsck bg-white"}`}>
                    <button className={`btn btn-sm btn-circle btn-ghost absolute right-2 top-2 ${theme == 'dark' ? " text-white bg-black" : ""}`}>✕</button>
                    <h1 className='text-blue-700 font-bold'>Add Manager</h1>
                    <div className='w-full mt-2'>
                        <form className='w-[100%]' >
                    ...
                    {loader ?
                        ...
                        <>
                            ...
                            <div className={`${theme == 'dark' ? " focus:outline-none border-none text-white " : "text-black bg-white"}`}>
                                ...
                                <h1 className='text-center'>Manager Profile</h1>
                                <hr></hr>
