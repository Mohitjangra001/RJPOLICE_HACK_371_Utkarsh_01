...
const Cofeedback = () => {
    ...
    const [emptyStatement, setEmptyStatement] = useState("No Services available"); // Set your default statement here
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (subject == "") {
            toast.error(`Dear ${cusername} Your are not allow to give feedback as your District has Disabled by Manager`)
        }
        if (feedbackData == "") {
    ...
    useEffect(() => {
        ...
        if (on === "nok") {
            navigate("/");
            toast.error(`Dear ${cusername}, you are not allowed to give feedback as your District has been disabled by the Manager`);
        }
        quesions();
        if (on === "ok") {
    ...
    return (
        <div className={`h-[91vh] overflow-y-auto w-full ${theme == 'light' ? 'text-black bg-white' : 'bg-[#1d232a]'}`}>
            ...
            <section className='flex flex-col sm:flex-row items-center px-5  justify-between mb-2 mt-5'>
                ...
                <Select className='w-[50vh] ant-input text-xl px-2 text-black' placeholder='Select a Service' onChange={handlechange}>
                    {sub.map((item, index) => {
